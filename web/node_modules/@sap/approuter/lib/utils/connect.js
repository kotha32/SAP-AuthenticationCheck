  ...
  const handle = (req, res, out) => {
    ...
    const next = (err) => {
      ...
      if (i < queue.length) {
        ...
        try {
          req.url = reqOrigin + ((u => u[0] === '/' ? u : '/' + u)(reqPath.slice(queue[i][0].route.length)));
    ...
    let args = [req, res, next], queue = okQueue, isFQDN = (req.url[0] !== '/'), lastError = null, i = 0;
    let pathStart = 0, reqOrigin = '', reqPath = req.url, reqPathLowerCase;
    if (isFQDN) {
      ...
      reqOrigin = reqPath.slice(0, pathStart);
